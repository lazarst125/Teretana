...
public sealed class PravilaPristupaTestovi : KomponentniTest
{
    ...
    private static HttpRequestMessage Zahtev(string metod, string sablonPutanje, Termin termin, Rezervacija rezervacija)
    {
        ...
        var ispravnoTelo = sablonPutanje.EndsWith("/prisustvo", StringComparison.Ordinal)
            ? (object)new { prisustvovao = true }
            : new { naziv = "Pokušaj bez prava", pocetak = TestniEntiteti.Sada.AddDays(5), kraj = TestniEntiteti.Sada.AddDays(5).AddHours(1), kapacitet = 5 };
